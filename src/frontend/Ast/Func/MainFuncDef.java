...
public class MainFuncDef extends Node{
    ...
    public MainFuncDef(int startLine, int endLine, SyntaxVarType syntaxVarType, ArrayList<Node> children) {
    ...
    public FuncSymbol createSymbol() {
        ...
        SymbolType symbolType = SymbolType.MainFunc;
    ...
    public void visit() {
        ...
        if(!abs) {
            ErrorRecorder.addError(new error.Error(error.Error.ErrorType.b, children.get(1).getStartLine()));
        }
        ...
        if(symbol.getReturnType() == ValueType.INT && !(last instanceof ReturnStmt)) {
            ErrorRecorder.addError(new error.Error(Error.ErrorType.g, endLine));
        }
    }

    //Main函数的方法和普通函数的操作是一样的
    @Override
    public Value genIR() {
        SymbolManager.getManager().setGlobal(false);
