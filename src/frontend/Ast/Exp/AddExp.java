    ...
    public Value genIR() {
        ...
        for(int i = 1; i < children.size(); i++) {
            if(children.get(i) instanceof TokenNode && (((TokenNode) children.get(i)).getToken().getTokenType()) == TokenType.PLUS) {
                ...
                instr = new AluInstr(IRBuilder.getInstance().genVarName(), AluInstr.Op.add, operand1, operand2);
                operand1 = instr;
            } else {
                i = i + 1;
                operand2 = children.get(i).genIR();
                instr = new AluInstr(IRBuilder.getInstance().genVarName(), AluInstr.Op.sub, operand1, operand2);
                operand1 = instr;
            }
        }
